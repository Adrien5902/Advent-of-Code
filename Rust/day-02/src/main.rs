...
fn main() {
    ...
    let games_sets: Vec<Game> = games.iter().enumerate().map(|(id, game)| {
        ...
        let subsets: Game = sets.iter().map(|set|{
            return set.split(", ").collect::<Vec<&str>>().iter().map(|subset|{
                ...
                return Subset {
                    ...
                    count: u32::from_str_radix(subsets_values[0], 10).expect("can't convert number")
                };
            }).collect::<Set>()
        }).collect();
        return subsets;
    ...
    if part == 1 {
        let result = games_sets.iter().enumerate().map(|(id, game)| {
            ...
            let is_possible: bool = game.iter().map(|set|{
                set.iter().map(|subset| {
                    subset.count <= match subset.color {
                        Color::Red => 12,
                        Color::Green => 13,
                        Color::Blue => 14,
                    }
                }).collect::<Vec<bool>>().iter().map(bool::to_owned).reduce(|a, b| {
                    a && b
                }).unwrap()
            }).collect::<Vec<bool>>().iter().map(bool::to_owned).reduce(|a,b| {
                a && b
